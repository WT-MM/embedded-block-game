...
- South plot: a real three-bit ripple button counter wired into a constructed
  seven-segment decoder matrix. The west button advances the count, the side
  buttons reset the bits, the three nearby lamps expose Q0/Q1/Q2, and the
  south-east display shows the decoded value. To clear the counter manually,
  reset Q0, then Q1, then Q2.
- South-east display matrix: the Q/NQ literal buses, invalid-row torches,
  segment taps, repeaters, and lamps are all normal redstone parts fed by the
  counter outputs.
