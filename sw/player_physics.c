...
void player_update(Player *p, VoxelWorld *world, float wish_dir_x, float wish_dir_z, bool jump, bool shift, float dt) {
    /* 1. Calculate Target Velocities (Slewing logic) */
    ...
    /* 4. Axis-by-Axis Collision Detection and Resolution */
    p->is_grounded = false;
    ...
    if (p->vx != 0.0f) {
        ...
        if (!check_collision(world, next_x, p->y, p->z)) {
            ...
            p->vx = 0.0f; // Halt upon hitting a wall
        }
    }

    // Y-Axis (Vertical / Gravity)
    if (p->vy != 0.0f) {
        float next_y = p->y + p->vy * dt;
        if (!check_collision(world, p->x, next_y, p->z)) {
            p->y = next_y;
        } else {
            if (p->vy < 0.0f) p->is_grounded = true; // Landing on the ground
            p->vy = 0.0f; 
            p->y = roundf(p->y); // Snap to block boundary to avoid micro-clipping
        }
    }
    ...
    if (p->vz != 0.0f) {
        ...
        if (!check_collision(world, p->x, p->y, next_z)) {
            ...
            p->vz = 0.0f; // Halt upon hitting a wall
        }
    }
}
