...
static bool get_abs_range(int fd, unsigned int axis, int *min_out,
                          int *max_out) {
...
static bool is_pointer_device(int fd) {
  return test_evbit(fd, EV_KEY) &&
         (test_keybit(fd, BTN_MOUSE) || test_keybit(fd, BTN_LEFT) ||
...
static bool is_relative_pointer(int fd) {
  ...
  return test_evbit(fd, EV_REL) && test_relbit(fd, REL_X) &&
...
static bool is_absolute_pointer(int fd) {
  ...
  return test_evbit(fd, EV_ABS) && test_absbit(fd, ABS_X) &&
...
static void add_pointer(InputState *inp, int fd, InputPointerMode mode) {
  ...
  fprintf(stderr, "input: pointer ready (%s, %s%s)\n",
          mode == INPUT_POINTER_REL ? "relative" : "absolute", name,
...
static void find_pointers(InputState *inp) {
  ...
  if (inp->_pointer_count == pointers_before || allow_absolute_with_relative) {
    scan_pointers(inp, INPUT_POINTER_ABS);
  }

  if (pointers_after_relative > pointers_before &&
      inp->_pointer_count > pointers_after_relative) {
    fprintf(stderr, "input: hybrid pointer mode active; relative motion "
                    "preferred with absolute fallback\n");
    ...
    fprintf(stderr, "input: relative pointer mode active; absolute tablet "
                    "input disabled by config\n");
  } else if (inp->_pointer_count > pointers_before &&
             inp->_pointers[pointers_before].mode == INPUT_POINTER_REL) {
    fprintf(stderr, "input: relative pointer mode active\n");
  } else if (inp->_pointer_count > pointers_before &&
             inp->_pointers[pointers_before].mode == INPUT_POINTER_ABS) {
    fprintf(stderr, "input: using absolute pointer fallback; VM window edges "
                    "may still limit look\n");
  }
}
...
static void text_queue_push(InputState *inp, char ch) {
  if (inp->text_queue_len >= INPUT_TEXT_QUEUE_MAX)
    return;
...
static void drain_fd(InputState *inp, int fd, InputPointer *pointer) {
  ...
  while (read(fd, &ev, sizeof(ev)) == sizeof(ev)) {
    ...
    } else if (pointer && pointer->mode == INPUT_POINTER_REL &&
               ev.type == EV_REL) {
      if (ev.value != 0)
    ...
    } else if (pointer && pointer->mode == INPUT_POINTER_ABS &&
               ev.type == EV_ABS) {
      if (ev.code == ABS_X) {
        float delta = 0.0f;
        ...
        if (pointer->have_abs_x) {
          delta = scale_abs_delta(ev.value - pointer->last_abs_x,
                                  pointer->abs_x_min, pointer->abs_x_max,
                                  ABS_MOUSE_SPAN_X) *
                  inp->_mouse_scale_x;
          have_delta = true;
        }
      ...
      if (ev.code == ABS_Y) {
        ...
        if (pointer->have_abs_y) {
          delta = scale_abs_delta(ev.value - pointer->last_abs_y,
                                  pointer->abs_y_min, pointer->abs_y_max,
                                  ABS_MOUSE_SPAN_Y) *
                  inp->_mouse_scale_y;
          have_delta = true;
        }
...
void input_clear_text_queue(InputState *inp) { inp->text_queue_len = 0; }
