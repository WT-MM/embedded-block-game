Archived Debug and Performance History
======================================

This file preserves the full pre-cleanup `PROJECT_NOTES.md` ledger. Keep new
current notes in the top-level `PROJECT_NOTES.md`; add long debugging narratives
here only when they are no longer part of the active working set.
...
May 2026: Cleanup / Row-Window Restore Notes
--------------------------------------------

The post-`rbf` cleanup intentionally keeps the latest generated Quartus/RBF
artifacts from `78d05e6` while restoring the simpler row-window software/RTL
ABI. `BAND_WINDOW` is again y-only:

  * `sw/voxel_gpu.h::voxel_band_state` contains `band_index`,
    `flush_y_min`, and `flush_y_max`.
  * `sw/voxel_gpu.c::voxel_ioc_begin_band` validates only local band rows and
    writes `[5:0]=y0`, `[13:8]=y1`.
  * `hw/voxel_gpu/rtl/voxel_gpu.sv` flushes full-width row windows; the
    dirty-rectangle x-range/segmented-write path is removed.
  * `sw/gpu_transport.c` still tracks touched scene rows so cached bands can
    use a smaller y window, but the hardware always flushes those rows at full
    screen width.

Source comments should now stay short and point here for history. Durable
details that previously lived inline:

  * The per-band primer descriptor is still intentional. It protects the first
    rasterized pixel after `BEGIN_BAND` from stale palette pipeline state. When
    a band is primer-only, software submits no descriptor bytes; hardware
    sky-gradient/clear init already fills the cache and no first-pixel draw
    hazard exists.
  * `BEGIN_BAND` returns after programming registers plus an MMIO readback. It
    deliberately does not wait for cache init; descriptor writes may fill the
    FIFO while hardware prepares the band.
  * The SDRAM background flush stages cache words into the controller WR FIFO
    before external write windows. `COPY_WR_FIFO_HIGH_WATER=224` leaves
    headroom in the 512-word FIFO while allowing scanout-read time to be used
    for internal FIFO staging. `COPY_DRAIN_CYCLES` is only the final
    post-empty settling wait; FIFO-empty is the flip-safety gate.
  * `CLEAR_FRAME` must not reset background flush or scanout read pipeline
    state. It is a software frame-begin command and can arrive during active
    display; killing in-flight SDRAM work produces partial frames or corrupt
    line fills.
  * `band_flush_pending` stays out of `BSY`. Ordering is enforced by the FSM
    priority chain and `band_begin_cache_available`, so the driver can pipeline
    bands without serializing on every background flush.

Code-quality cleanup in the same pass:

  * Cube-face occlusion rules now live in
    `sw/block_types.c::block_face_should_render`. Both chunk meshing and the
    renderer debug/block-array path call the same helper, so translucent and
    alpha-keyed block visibility should not drift between paths.
  * Removed unused `world_get_sky_light` / `world_set_sky_light` helpers. The
    optimized lighting rebuild walks chunk arrays directly.
  * `sw/Makefile` has a shared `TEST_TARGETS` list, shared renderer-test
    source/dependency variables, and `make clean_tests` for desktop cleanup
    without invoking kernel-module cleanup.

Cleanup validation used the desktop-safe suite:

  * `make -C sw tests`
  * Verilator lint of the touched RTL with vendor primitive warnings suppressed
  * `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run pytest` in `virtual_hw`
  * socket-backed renderer smoke tests against `virtualhw --headless`
...
Historical Architecture Idea: True 640x480 via Direct-to-SDRAM Z-Cache
----------------------------------------------------------------------

Before the SDRAM band-cache work, the renderer was limited to a 320x240
internal resolution upscaled 2x2 because a single 16-bit RGB565 backbuffer and
16-bit Z-buffer exhausted the FPGA's internal M10K block RAM. A full 640x480
internal framebuffer would require about 1.2 MB, which does not fit on the
Cyclone V.
...
band cache:
  ...
  * Cache shape: 640x60 pixels.
  * Cache contents: RGB565 color cache + 16-bit z cache.
  * Hot-path storage cost: 640 * 60 * 4 bytes = 150 KiB.
...
Default byte layout for the first pass:
  ...
  * back color frame: byte base `1048576`
  * z backing frame: byte base `2097152`
...
Implement as one coherent RTL change, not as a half-swapped framebuffer:
  ...
  2. Replace full-frame `fb_back_ram` and `z_ram` with 640x60 band RAMs.
...
What changed:
  ...
  * The full-frame BRAM color and z memories were replaced with 640x60 band
    memories.
...
Bring-up caveats:
  ...
  * The z backing base is currently fixed at byte address `2097152`
    in RTL rather than exposed as its own CSR.
...
arbitration. Given M10K pressure from ping-pong 640x60 bands, this is only
